...
D:\WebRelated\YUICompressor\build>java -jar yuicompressor-2.4.2.jar d:\Inetpub\wwwroot\sweetAndSour\js\common.js -o d:\Inetpub\wwwroot\sweetAndSour\js\common.min.js 
*/
...
var SweetAndSour = (function() { 
	...
	function _objFindPos(obj) {
		...
		//console.log("_objFindPos x: " + curLeft + ", y: " + curTop);
	...
	// When window is dragged, store the new apparent position (in the visible screen, not from top of page)
	...
	// Insert a break after every third photo in a ph
	function _alignPhotos(photoAlbums) {
		var album, photos, numPhotos, photo, index;
		photoAlbums.each(
			function(album){
				...
				photos.each(
					function(photo, index) {
						if(((index + 1)%3 == 0) && (index != numPhotos - 1)) {
	...
	return {  
		...
		// Initialize page
		initialize : function() {
			// If IE or Opera, set <span>s in div#page to give rounded corners (FF & Safari can use CSS).
			...
			// As the drag bar is moved, need to simultaneously move the window (right edge will appear fixed)
			...
			*/
			...
			btnClose.addEvent("mouseup", function() {btnClose.className = ""});
			
			// Scroll DHTML with real window (or it won't be visible if user opens photo some way down the page);
		...
		getPhoto : function(photoName, thumbnailCaption, panorama) {
			...
			if(screenW < 800) {
				...
				newURL = "../imageMgt/index.php?fuseAction=showPhotoAndCaption&photoName=" + photoName + (panorama ? "&panorama=true" : "");
				newWindow = window.open(newURL, 'newWindow', 'width=' + screenW + ',height=' + screenH + ',left=0,top=0,dependant=yes,directories=no,location=no,menubar=no,resizable=yes,scrollbars=yes,status=no,titlebar=yes,toolbar=no');
				newWindow.focus();
			}
			else {
...
//google.load("search", "1", {"nocss":true, "language":"en"});
...
function OnLoad() {
	...
	*/
	...
	btn.addEvent("click", function(event) {SearchAdditions.closeResults(event)});
}
